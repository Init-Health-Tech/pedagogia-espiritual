      ...
      <Container maxWidth={false} sx={{ maxWidth: 760, mx: 'auto', px: 3 }}>
        <Grid container spacing={4} sx={{ mb: 4 }}>
          <Grid size={{ xs: 12, md: 5 }}>
            <Box sx={{ mb: 2 }}><TorLogo size="sm" /></Box>
              ...
              <Stack spacing={1}>
                {col.links.map((link) =>
                  link.route ? (
                    ...
                    <Link key={link.label} href={link.href} underline="hover" onClick={(e) => { e.preventDefault(); scrollTo(link.href) }} sx={{ color: colors.primary, fontSize: '1rem', cursor: 'pointer' }}>
                      {link.label}
                    </Link>
                  ),
                )}
